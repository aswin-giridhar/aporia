...
**1. A single agent's confidence in its own answer did not track its
correctness.** Baseline calibration AUC came out **below 0.5 on every model —
0.062 / 0.000 / 0.125**. Directionally this is the expected story: self-reported
confidence, generated by the same process that produced the error, was if
anything inversely related to being right. **Caveat, stated up front:** n = 6
tasks per model, so this is a suggestive direction consistent across three
tiers, not a proven effect size. We report it as such.
...
This is stated as what it is — a limitation, not a feature.

**On the society's calibration AUC — do not over-read it.** The reported values
(0.75 / 0.50 / 0.75) are **not** evidence of a working uncertainty signal, and
we will not claim they are. Two reasons, both disqualifying on their own:

1. **The metric is saturated.** Five of six per-task disagreement scores sit at
   exactly 0.85, because the escalation policy floors disagreement at 0.85
   whenever it fires. So "disagreement predicts escalation" is close to
   circular — the score is partly *set by* the decision it is supposed to
   predict. The single clean data point (one solvable task that escaped the
   floor) is what moves the AUC off 0.5.
2. **n = 6.** With a 4-vs-2 split, one task swings the AUC by 0.25. This is far
   too small to call anything "calibrated". The honest read of 0.75 / 0.50 /
   0.75 is "inconclusive", not "it works".

The *architecture* for an uncertainty signal is in place; whether inter-agent
disagreement is actually a reliable predictor of failure is **not yet
demonstrated** by this run, and a larger, un-saturated evaluation is required to
say either way. A benchmark that could only flatter its authors would not have
made this visible; ours does, and it refuted two of our own design choices along
the way (see the commit history).
